...
class Modal extends Component {
    ...
    render() {
        const { type = 'modal', prefixCls, title, content, cancelBtnText, doneBtnText, children } = this.props;
        ...
        const tContent = (
            <div className={`${prefixCls} ${isShow ? 'show' : ''}`}>
                {hasMask && <div className={`${prefixCls}-mask`} onClick={this.handleCancel} />}
                <div className={`${prefixCls}-content ${tModalType}`}>
                    <div className={`${prefixCls}-header border-b`}>
                        <span className={`nowrap ${prefixCls}-title`}>{title || '标题'}</span>
                    </div>
                    {<div className={`${prefixCls}-body`}>{content || children || '暂无内容'}</div>}
                    {type !== 'layer' && (
                        <div className={`${prefixCls}-footer border-t`}>
                            <div className="btnWrap btn-group">
                                ...
                                {doneTxt && (
                                    <Button size="xs" onClick={this.handleDone}>
                                        {doneTxt}
                                    </Button>
                                )}
                            </div>
                    ...
                    {hasMask && (
                        <span className={`${prefixCls}-closeIcon`}>
                            <CloseIcon size="lg" handleClick={this.handleCancel} />
                        </span>
                    )}
                </div>
            </div>
...
Modal.propTypes = {
    prefixCls: PropTypes.oneOf(['modal', 'alert', 'layer']),
    // 弹出框标题，默认值：“”
